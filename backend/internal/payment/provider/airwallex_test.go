...
// TestNewAirwallexDerivesAPIBaseFromLegacyEnvironment covers the downstream
// backward-compat shim: provider instances created before the upstream sync
// stored an `environment` (demo|prod) key instead of an explicit `apiBase`.
// NewAirwallex must keep accepting those configs.
func TestNewAirwallexDerivesAPIBaseFromLegacyEnvironment(t *testing.T) {
	t.Parallel()

	// Legacy prod config: no apiBase, environment=prod.
	prov, err := NewAirwallex("1", map[string]string{
		"clientId":      "cid",
		"apiKey":        "key",
		"webhookSecret": "secret",
	...
	require.NoError(t, err)
	require.Equal(t, airwallexProdAPIBase, prov.config["apiBase"])
	...
	require.NoError(t, err)
	require.Equal(t, airwallexDemoAPIBase, prov.config["apiBase"])

	// An explicit apiBase always wins over a stale environment value.
	prov, err = NewAirwallex("1", map[string]string{
		"clientId":      "cid",
		"apiKey":        "key",
		"webhookSecret": "secret",
		"environment":   "demo",
		"apiBase":       airwallexProdAPIBase,
	})
	require.NoError(t, err)
	require.Equal(t, airwallexProdAPIBase, prov.config["apiBase"])

	// Neither apiBase nor a recognizable environment → still rejected.
	_, err = NewAirwallex("1", map[string]string{
		"clientId":      "cid",
		"apiKey":        "key",
		"webhookSecret": "secret",
