    ...
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
        ...
        <div class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-gray-900 dark:text-white">{{ formatCreditedAmount(order?.amount) }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
        ...
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          ...
          <span class="font-medium text-red-600 dark:text-red-400">{{ formatCreditedAmount(actuallyRefunded) }}</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          ...
          <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
            ...
            <div class="mt-1 font-semibold text-gray-900 dark:text-white">{{ formatCreditedAmount(order?.amount) }}</div>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ creditedSymbol }}</span>
          <input
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {{ t('payment.admin.maxRefundable') }}: {{ formatCreditedAmount(maxRefundable) }}
        </p>
      </div>
...
import { formatPaymentAmount, paymentCurrencySymbol } from '@/components/payment/currency'
...
const creditedSymbol = computed(() => {
  if (props.order?.order_type === 'balance') return '$'
  return paymentCurrencySymbol(props.order?.currency)
})

function formatCreditedAmount(value: number | null | undefined): string {
  const numeric = Number(value) || 0
  if (props.order?.order_type === 'balance') return '$' + numeric.toFixed(2)
  return formatPaymentAmount(numeric, props.order?.currency)
}
